...
const updateTaskCheckList = async (req, res) => {
  try {
    ...
    // Auto-update based on the checklist completion
    const completedCount = task.todoCheckList.filter((item) => item.Completed).length;
    const taskItems = task.todoCheckList.length;
    task.progress =totalItems > 0 ? Math.round((completedCount / taskItems) * 100) : 0;

    // Auto-maark task as completd if all items are completed
